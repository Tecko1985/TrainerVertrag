...
async function _zeigeTab(tab) {
  ...
  const dlFlow      = document.getElementById("downloads-flow");
  const connect     = document.getElementById("admin-connect-screen");
  const panel       = document.getElementById("admin-panel");

  if (tab === "meine") {
    adminFlow.style.display = "none";
    dlFlow.style.display = "none";
    trainerFlow.style.display = "";
    document.getElementById("file-status").style.display = "none";
    return;
  }

  // Downloads braucht weder Verbindung noch Recht — der submit-worker prüft den
  // Login selbst, wie im ganzen Trainer-Bereich. Bei jedem Öffnen frisch laden:
  // eine Freigabe kann seit dem letzten Blick dazugekommen sein.
  if (tab === "downloads") {
    adminFlow.style.display = "none";
    trainerFlow.style.display = "none";
    dlFlow.style.display = "";
    document.getElementById("file-status").style.display = "none";
    _ladeDownloads();
    return;
  }
  ...
  // Erst hier rendern, nicht beim Verbinden: vorher steht appData.downloads
  // noch nicht fest, und die Karten stehen ohnehin nur in diesem Tab.
  if (tab === "einstellungen") _renderDownloadsVerwaltung();
}
...
function _initAdminPanel() {
  ...
  document.getElementById("btn-d-efz-freigeben").addEventListener("click", () => _setzeEfzFreigabe(true));
  document.getElementById("btn-d-efz-entziehen").addEventListener("click", () => _setzeEfzFreigabe(false));
}
...
//
// Zwei Seiten desselben Features:
//   Trainer  -> Tab „Downloads": Dateien des Vereins + eigenes Bestätigungsschreiben
//   Admin    -> Einstellungen-Tab (Dateien, Absender, Stempel) + Trainerliste (Freigabe)
//
// Der Trainer-Weg läuft komplett über den submit-worker (Bearer-Token), der
// Admin-Weg über den bestehenden WebDAV-Zugang. Deshalb braucht der Tab weder
// Verbindung noch Recht und steht auch Konten ohne Vertragspflicht offen.

let _downloadsStand = null; // letzte Antwort von my-downloads (für den Brief-Knopf)

function _downloadsBlock() {
  if (!appData.downloads || typeof appData.downloads !== "object") appData.downloads = {};
  if (!Array.isArray(appData.downloads.dateien)) appData.downloads.dateien = [];
  if (!appData.downloads.efz || typeof appData.downloads.efz !== "object") appData.downloads.efz = {};
  return appData.downloads;
}

// ── Trainer-Sicht ─────────────────────────────────────────────────────────────

async function _ladeDownloads() {
  const fehlerEl = document.getElementById("dl-fehler");
  fehlerEl.style.display = "none";
  try {
    _downloadsStand = await fetchMyDownloads();
  } catch (err) {
    _downloadsStand = null;
    fehlerEl.textContent = err instanceof NotLoggedInError
      ? "Deine Sitzung ist abgelaufen. Bitte in der Tools-Übersicht neu anmelden."
      : "Downloads konnten nicht geladen werden: " + err.message;
    fehlerEl.style.display = "block";
    return;
  }
  _renderDownloads(_downloadsStand);
}

function _renderDownloads(stand) {
  const efz    = (stand && stand.efz) || {};
  const card   = document.getElementById("dl-efz-card");
  const btn    = document.getElementById("btn-dl-efz");
  const hint   = document.getElementById("dl-efz-hinweis");
  const geholt = document.getElementById("dl-efz-geholt");

  card.style.display = efz.freigegeben ? "" : "none";
  if (efz.freigegeben) {
    const fehlend = Array.isArray(efz.fehlend) ? efz.fehlend : [];
    // Zwei getrennte Gründe für einen gesperrten Knopf, und beide gehören
    // benannt: bei fehlenden eigenen Angaben kann die Person selbst etwas tun,
    // bei einer unfertigen Vereinsseite nicht.
    if (fehlend.length) {
      btn.disabled = true;
      hint.textContent = "Dafür fehlt bei dir noch: " + fehlend.join(", ") +
        ". Trag das unter Meine Daten nach, dann kannst du das Schreiben herunterladen.";
      hint.style.display = "block";
    } else if (!efz.bereit) {
      btn.disabled = true;
      hint.textContent = "Die Geschäftsstelle hat das Schreiben noch nicht fertig eingerichtet. Bitte später erneut versuchen.";
      hint.style.display = "block";
    } else {
      btn.disabled = false;
      hint.style.display = "none";
    }
    if (efz.zuletztGeholtAm) {
      geholt.textContent = "Zuletzt heruntergeladen am " + _fmtIso(efz.zuletztGeholtAm);
      geholt.style.display = "block";
    } else {
      geholt.style.display = "none";
    }
  }

  const dateien = Array.isArray(stand && stand.dateien) ? stand.dateien : [];
  const rows    = document.getElementById("dl-dateien-rows");
  const empty   = document.getElementById("dl-dateien-empty");
  empty.style.display = dateien.length ? "none" : "";
  rows.innerHTML = dateien.map(d => `
    <div class="doc-row">
      <div class="doc-row-main">
        <span class="doc-row-label">${_esc(d.name)}</span>
        ${d.beschreibung ? `<span class="muted">${_esc(d.beschreibung)}</span>` : ""}
        <span class="muted" style="font-size:12px;">${_esc(d.dateiName)}${d.groesse ? " · " + _dlGroesse(d.groesse) : ""}</span>
      </div>
      <div class="btn-row" style="justify-content:flex-start;">
        <button type="button" class="btn small" data-dl-id="${_esc(d.id)}">Herunterladen</button>
      </div>
    </div>`).join("");

  rows.querySelectorAll("button[data-dl-id]").forEach(b => {
    b.addEventListener("click", () => _holeDownloadDatei(b.dataset.dlId, b));
  });
}

function _dlGroesse(bytes) {
  if (bytes >= 1024 * 1024) return (bytes / 1024 / 1024).toFixed(1).replace(".", ",") + " MB";
  return Math.max(1, Math.round(bytes / 1024)) + " KB";
}

// Öffnet die Datei in einem neuen Tab statt einen Download zu erzwingen: ein PDF
// will man meist erst ansehen, und auf dem iPhone ist ein <a download> auf eine
// Blob-URL wirkungslos. Das leere Fenster muss synchron aufgehen, sonst blockt
// Safari es nach dem await (siehe _openBlobTab).
async function _holeDownloadDatei(id, btn) {
  const tab = _openBlobTab();
  const fehlerEl = document.getElementById("dl-fehler");
  fehlerEl.style.display = "none";
  btn.disabled = true;
  try {
    const blob = await fetchDownloadDatei(id);
    tab.show(blob);
  } catch (err) {
    tab.abort();
    fehlerEl.textContent = "Datei konnte nicht geladen werden: " + err.message;
    fehlerEl.style.display = "block";
  } finally {
    btn.disabled = false;
  }
...
async function _holeEfzBrief() {
  ...
  try {
    const daten = await fetchEfzBriefDaten();
    const blob  = await baueEfzBriefPdf(daten);
    tab.show(blob);
    // Der Zeitstempel „zuletzt geholt" ist gerade serverseitig gesetzt worden —
    // ohne Nachladen stünde in der Karte weiter der alte Stand.
    _ladeDownloads();
  } catch (err) {
    tab.abort();
    fehlerEl.textContent = err instanceof NotLoggedInError
      ? "Deine Sitzung ist abgelaufen. Bitte in der Tools-Übersicht neu anmelden."
      : "Schreiben konnte nicht erstellt werden: " + err.message;
    fehlerEl.style.display = "block";
  } finally {
    btn.disabled = false;
    btn.textContent = "Mein Bestätigungsschreiben (PDF)";
  }
}

// ── Admin: Dateien und Einrichtung (Einstellungen-Tab) ────────────────────────

function _initDownloadsAdmin() {
  document.getElementById("btn-dl-efz").addEventListener("click", _holeEfzBrief);

  document.getElementById("btn-dlv-upload").addEventListener("click", () => document.getElementById("dlv-file-input").click());
  document.getElementById("dlv-file-input").addEventListener("change", (e) => {
    const f = e.target.files[0]; e.target.value = "";
    if (f) _downloadDateiHochladen(f);
  });
  document.getElementById("btn-efz-speichern").addEventListener("click", _efzEinstellungenSpeichern);
  document.getElementById("btn-efz-stempel").addEventListener("click", () => document.getElementById("efz-stempel-input").click());
  document.getElementById("efz-stempel-input").addEventListener("change", (e) => {
    const f = e.target.files[0]; e.target.value = "";
    if (f) _efzStempelHochladen(f);
  });
  document.getElementById("efz-einsatz-standard").placeholder = EFZ_EINSATZ_PLATZHALTER;
}

// Ablage der Downloadbereich-Dateien: Geschwister von trainerdaten.json, genau
// wie fuehrerscheine/ und die anderen Dokumentordner. Das Stempelbild liegt
// bewusst in einem EIGENEN Ordner — so kann die Trainer-Aktion "download-datei"
// es gar nicht erst adressieren.
function _downloadsDateiConfig(id) {
  return _trainerDocConfig("downloads", id);
}
function _efzStempelConfig() {
  return _trainerDocConfig("efz", "stempel");
}

async function _downloadDateiHochladen(file) {
  const fehlerEl = document.getElementById("dlv-fehler");
  const statusEl = document.getElementById("dlv-status");
  fehlerEl.style.display = "none";
  const block = _downloadsBlock();

  if (block.dateien.length >= DOWNLOADS_MAX_DATEIEN) {
    fehlerEl.textContent = "Es sind höchstens " + DOWNLOADS_MAX_DATEIEN + " Dateien möglich. Bitte zuerst eine entfernen.";
    fehlerEl.style.display = "block";
    return;
  }
  if (file.size > MAX_FILE_BYTES) {
    fehlerEl.textContent = "Datei ist zu groß (höchstens " + Math.round(MAX_FILE_BYTES / 1024 / 1024) + " MB).";
    fehlerEl.style.display = "block";
    return;
  }

  const id = crypto.randomUUID();
  const name = document.getElementById("dlv-name").value.trim() || file.name;
  const beschreibung = document.getElementById("dlv-beschreibung").value.trim();

  statusEl.textContent = "Lädt hoch …";
  try {
    await davWriteBinary(_downloadsDateiConfig(id), file, file.type || "application/octet-stream");
  } catch (err) {
    statusEl.textContent = "";
    fehlerEl.textContent = "Upload fehlgeschlagen: " + err.message;
    fehlerEl.style.display = "block";
    return;
  }

  // Erst Datei, dann Metadaten — dieselbe Reihenfolge wie bei den Dokument-
  // Uploads. Bricht Schritt 2 ab, liegt eine Datei ohne Verweis herum; andersherum
  // stünde ein Eintrag in der Liste, dessen Datei es nicht gibt.
  block.dateien.push({
  ...
  try {
    ...
    statusEl.textContent = "";
    fehlerEl.textContent = "Datei hochgeladen, aber Speichern der Liste fehlgeschlagen: " + err.message;
    fehlerEl.style.display = "block";
  }
  document.getElementById("dlv-name").value = "";
  document.getElementById("dlv-beschreibung").value = "";
  _renderDownloadsVerwaltung();
}

async function _downloadDateiEntfernen(id) {
  const block = _downloadsBlock();
  const eintrag = block.dateien.find(d => d && d.id === id);
  if (!eintrag) return;
  const bez = eintrag.name || eintrag.dateiName || "Datei";
  if (!confirm(bez + " aus dem Downloadbereich entfernen?")) return;

  const fehlerEl = document.getElementById("dlv-fehler");
  fehlerEl.style.display = "none";
  // Erst die Datei, dann der Eintrag: bleibt Schritt 2 aus, zeigt die Liste einen
  // Eintrag, dessen Download 404 liefert — sichtbar und beim nächsten Versuch
  // heilbar. Andersherum bliebe die Datei unauffindbar liegen.
  try {
    await davDeleteFile(_downloadsDateiConfig(id));
  } catch (err) {
    fehlerEl.textContent = "Datei konnte nicht gelöscht werden: " + err.message;
    fehlerEl.style.display = "block";
    return;
  }
  block.dateien = block.dateien.filter(d => d && d.id !== id);
  try {
    await _saveMerged();
  } catch (err) {
    fehlerEl.textContent = "Datei gelöscht, aber Speichern der Liste fehlgeschlagen: " + err.message;
    fehlerEl.style.display = "block";
  }
  _renderDownloadsVerwaltung();
}

function _renderDownloadsVerwaltung() {
  const block = _downloadsBlock();
  const rows  = document.getElementById("dlv-rows");
  const empty = document.getElementById("dlv-empty");
  if (!rows) return;
  empty.style.display = block.dateien.length ? "none" : "";
  rows.innerHTML = block.dateien.map(d => `
    <div class="doc-row">
      <div class="doc-row-main">
        <span class="doc-row-label">${_esc(d.name || d.dateiName || "Datei")}</span>
        ${d.beschreibung ? `<span class="muted">${_esc(d.beschreibung)}</span>` : ""}
        <span class="muted" style="font-size:12px;">${_esc(d.dateiName || "")}${d.groesse ? " · " + _dlGroesse(d.groesse) : ""} · seit ${_esc(_fmtIso(d.hochgeladenAm))}</span>
      </div>
      <div class="btn-row" style="justify-content:flex-start;">
        <button type="button" class="btn danger small" data-dlv-del="${_esc(d.id)}">Entfernen</button>
      </div>
    </div>`).join("");
  rows.querySelectorAll("button[data-dlv-del]").forEach(b => {
    b.addEventListener("click", () => _downloadDateiEntfernen(b.dataset.dlvDel));
  });

  const efz = block.efz;
  document.getElementById("efz-absender").value = efz.absender || "";
  document.getElementById("efz-einsatz-standard").value = efz.einsatzStandard || "";
  document.getElementById("efz-stempel-status").textContent = efz.stempelContentType
    ? "hinterlegt am " + _fmtIso(efz.stempelHochgeladenAm)
    : "noch keins hinterlegt";
  _ladeStempelVorschau();
}

async function _ladeStempelVorschau() {
  const img = document.getElementById("efz-stempel-vorschau");
  const efz = _downloadsBlock().efz;
  if (!efz.stempelContentType) { img.style.display = "none"; img.removeAttribute("src"); return; }
  try {
    const blob = await davReadBinary(_efzStempelConfig(), efz.stempelContentType);
    if (!blob) { img.style.display = "none"; return; }
    img.src = await _blobToDataUrl(blob);
    img.style.display = "";
  } catch (_) {
...
async function _efzStempelHochladen(file) {
  const fehlerEl = document.getElementById("efz-fehler");
  const statusEl = document.getElementById("efz-status");
  fehlerEl.style.display = "none";
  const ctype = (file.type || "").toLowerCase();
  // pdf-lib kann ausschließlich PNG und JPEG einbetten — ein anderes Format
  // ließe erst das Erzeugen des Briefs scheitern, also hier abfangen.
  if (!EFZ_STEMPEL_TYPEN.includes(ctype)) {
    fehlerEl.textContent = "Bitte ein PNG oder JPEG wählen — andere Formate lassen sich nicht in den Brief einbetten.";
    fehlerEl.style.display = "block";
    return;
  }
  if (file.size > MAX_FILE_BYTES) {
    fehlerEl.textContent = "Bild ist zu groß (höchstens " + Math.round(MAX_FILE_BYTES / 1024 / 1024) + " MB).";
    fehlerEl.style.display = "block";
    return;
  }
  ...
  statusEl.textContent = "Lädt hoch …";
  try {
    await davWriteBinary(_efzStempelConfig(), file, ctype);
  } catch (err) {
    statusEl.textContent = "";
    fehlerEl.textContent = "Upload fehlgeschlagen: " + err.message;
    fehlerEl.style.display = "block";
  ...
  const block = _downloadsBlock();
  block.efz.stempelContentType  = ctype;
  block.efz.stempelHochgeladenAm = new Date().toISOString();
  try {
    await _saveMerged();
    statusEl.textContent = "Stempelbild gespeichert ✓";
    setTimeout(() => { statusEl.textContent = ""; }, 2500);
  } catch (err) {
    statusEl.textContent = "";
    fehlerEl.textContent = "Bild hochgeladen, aber Speichern fehlgeschlagen: " + err.message;
    fehlerEl.style.display = "block";
  }
  _renderDownloadsVerwaltung();
}

async function _efzEinstellungenSpeichern() {
  const fehlerEl = document.getElementById("efz-fehler");
  const statusEl = document.getElementById("efz-status");
  fehlerEl.style.display = "none";
  const block = _downloadsBlock();
  block.efz.absender        = document.getElementById("efz-absender").value.trim();
  block.efz.einsatzStandard = document.getElementById("efz-einsatz-standard").value.trim();
  statusEl.textContent = "Speichere …";
  try {
    await _saveMerged();
    statusEl.textContent = "Gespeichert ✓";
    ...
    statusEl.textContent = "";
    fehlerEl.textContent = "Speichern fehlgeschlagen: " + err.message;
    fehlerEl.style.display = "block";
  }
}

// ── Admin: Freigabe je Person (Detailansicht) ─────────────────────────────────

function _renderEfzFreigabeRow(t) {
  const statusEl  = document.getElementById("d-efz-status");
  const einsatzEl = document.getElementById("d-efz-einsatz");
  const freiBtn   = document.getElementById("btn-d-efz-freigeben");
  const wegBtn    = document.getElementById("btn-d-efz-entziehen");
  if (!statusEl) return;

  const f = (t && typeof t.efzFreigabe === "object" && t.efzFreigabe) || null;
  einsatzEl.value     = f ? (f.einsatzbereich || "") : "";
  freiBtn.textContent = f ? "Einsatzbereich übernehmen" : "Freigeben";
  wegBtn.disabled     = !f;

  if (!f) {
    statusEl.textContent = "Nicht freigegeben — die Person sieht den Knopf nicht.";
    return;
  }
  let txt = "Freigegeben am " + _fmtIso(f.freigegebenAm);
  if (f.freigegebenVon) txt += " von " + f.freigegebenVon;
  txt += f.zuletztGeholtAm
    ? " · zuletzt geholt am " + _fmtIso(f.zuletztGeholtAm)
    : " · noch nicht heruntergeladen";
  statusEl.textContent = txt;
}

// Freigeben/Entziehen schreibt über den normalen WebDAV-Weg wie jede andere
// Änderung am Datensatz. Nur die Meldung ans Handy braucht den submit-worker —
// der schlägt den Empfänger selbst im Datensatz nach.
//
// ⚠️ Reihenfolge: erst speichern, dann melden. Andersherum bekäme die Person eine
// Nachricht über etwas, das noch nicht freigegeben ist, und der Worker wiese sie
// (zu Recht) mit 409 ab.
async function _setzeEfzFreigabe(frei) {
  if (!currentTrainerId) return;
  const idx = appData.trainer.findIndex(x => x.id === currentTrainerId);
  if (idx === -1) return;
  const errEl = document.getElementById("d-doc-error");
  ...
  const vorher = appData.trainer[idx].efzFreigabe;
  if (!frei) {
    if (!confirm("Freigabe entziehen? Die Person kann das Schreiben danach nicht mehr herunterladen.")) return;
    delete appData.trainer[idx].efzFreigabe;
  } else {
    appData.trainer[idx].efzFreigabe = {
      // Ein erneutes Freigeben ändert nur den Einsatzbereich; das ursprüngliche
      // Datum und ein bereits erfolgter Abruf bleiben stehen.
      freigegebenAm:   (vorher && vorher.freigegebenAm) || new Date().toISOString(),
      freigegebenVon:  currentUsername || "",
      einsatzbereich:  document.getElementById("d-efz-einsatz").value.trim(),
      zuletztGeholtAm: (vorher && vorher.zuletztGeholtAm) || ""
    };
  }
  ...
  try {
    ...
    // Die Anzeige darf keinen Zustand behaupten, der nicht gespeichert ist.
    if (vorher) appData.trainer[idx].efzFreigabe = vorher;
    else delete appData.trainer[idx].efzFreigabe;
    errEl.textContent = "Speichern fehlgeschlagen: " + err.message;
    errEl.style.display = "block";
    _renderEfzFreigabeRow(appData.trainer[idx]);
    return;
  }
  _renderEfzFreigabeRow(appData.trainer[idx]);

  // Nur bei einer NEUEN Freigabe melden — nicht beim Nachschärfen des
  // Einsatzbereichs und nicht beim Entziehen.
  if (frei && !vorher) {
    try {
      const r = await sendeEfzPush(currentTrainerId);
      if (r && r.infrage === 0) {
        errEl.textContent = "Freigegeben. Eine Meldung ging nicht raus (kein Konto verknüpft) — bitte selbst Bescheid geben.";
        errEl.style.display = "block";
      }
    } catch (err) {
      // Die Freigabe steht, nur die Meldung fehlt. Das gehört gesagt, ist aber
      // kein Fehlschlag der Handlung selbst.
      errEl.textContent = "Freigegeben. Die Meldung ans Handy ging nicht raus: " + err.message;
      errEl.style.display = "block";
    }
  }
}
